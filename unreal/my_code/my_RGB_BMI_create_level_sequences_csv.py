...
def add_geometry_cache(level_sequence, sequence_body_index, layer_suffix, start_frame, end_frame, target_object, x, y,
                       z, yaw, pitch, roll, material=None, texture_body_path=None, texture_clothing_overlay_path=None):
    """
    ...
    if texture_clothing_overlay_path is not None:
        ...
        geometry_cache_actor = unreal.get_editor_subsystem(unreal.EditorActorSubsystem).spawn_actor_from_class(
            clothing_actor_class, unreal.Vector(0, 0, 0))
        geometry_cache_actor.set_editor_property("bodytexture", unreal.SystemLibrary.conv_soft_obj_path_to_soft_obj_ref(
            unreal.SoftObjectPath(texture_body_path)))
        geometry_cache_actor.set_editor_property("clothingtextureoverlay",
                                                 unreal.SystemLibrary.conv_soft_obj_path_to_soft_obj_ref(
                                                     unreal.SoftObjectPath(texture_clothing_overlay_path)))
    else:
        geometry_cache_actor = unreal.get_editor_subsystem(unreal.EditorActorSubsystem).spawn_actor_from_class(
            unreal.GeometryCacheActor, unreal.Vector(0, 0, 0))
    ...
    geometry_cache_actor.get_geometry_cache_component().set_editor_property("looping",
                                                                            False)  # disable looping to prevent ghosting on last frame with temporal sampling
    ...
    unreal.get_editor_subsystem(unreal.EditorActorSubsystem).destroy_actor(
        geometry_cache_actor)  # Delete temporary template actor from level
...
def add_hair(level_sequence, sequence_body_index, layer_suffix, start_frame, end_frame, hair_path, animation_path, x, y,
             z, yaw, pitch, roll, ):
    """
    ...
    hair_actor = unreal.get_editor_subsystem(unreal.EditorActorSubsystem).spawn_actor_from_class(unreal.StaticMeshActor,
                                                                                                 unreal.Vector(0, 0, 0))
    ...
    unreal.get_editor_subsystem(unreal.EditorActorSubsystem).destroy_actor(
        skeletal_mesh_actor)  # Delete temporary template actor from level
    unreal.get_editor_subsystem(unreal.EditorActorSubsystem).destroy_actor(
        hair_actor)  # Delete temporary template actor from level
            ...
            for channel in section.get_channels():
                ...
                if len(channel_keys) > 0:
                    if len(channel_keys) != 2:  # only change end keyframe time if channel has two keyframes
                        unreal.log_error(
                            "WARNING: Channel does not have two keyframes. Not changing last keyframe to sequence end frame.")
                    else:
                        end_key = channel_keys[1]
                        end_key.set_time(unreal.FrameNumber(new_frame))


def add_level_sequence(name, camera_actor, camera_pose, ground_truth_logger_actor, sequence_bodies, sequence_frames,
                       hdri_name, camera_hfov=None, camera_movement="Static", cameraroot_yaw=None,
    ...
    if hdri_name is not None:
        ...
        level_sequence = unreal.AssetTools.create_asset(asset_tools, asset_name=name, package_path=level_sequences_root,
    ...
    for sequence_body_index, sequence_body in enumerate(sequence_bodies):
        ...
        if sequence_body.texture_clothing_overlay is not None:
            ...
            add_geometry_cache(level_sequence, sequence_body_index, "body", animation_start_frame, animation_end_frame,
                               body_object, sequence_body.x, sequence_body.y, sequence_body.z, sequence_body.yaw,
                               sequence_body.pitch, sequence_body.roll, None, texture_body_path,
                               texture_clothing_overlay_path)

        else:
            # Add body
            material = None

            # Assign material based on predefined conditions
            material_path = None

            try:
                subject_BMI = int(
                    sequence_body.subject.split('_')[1])  # Extract second element and convert to int
            except (IndexError, ValueError):
                unreal.log_error(f"Invalid subject format: {sequence_body.subject}")
                return False

            unreal.log(f"OLAAAAAAAAAAAAA {sequence_body.subject}, {subject_BMI}")


            # sequence_body.subject
            if 25 <= subject_BMI < 33:  # Normal weight
                material_path = "/Game/M_Green"
            elif 33 <= subject_BMI < 40:  # Obese
                material_path = "/Game/M_Blue"
            elif 40 <= subject_BMI < 60:  # Severe Obese
                material_path = "/Game/M_Red"

            # Load the material if a valid path is assigned
            if material_path:
                material = unreal.EditorAssetLibrary.load_asset(material_path)
                if not material:
                    unreal.log_error(f"Cannot load material: {material_path}")
                    return False

            # Add the geometry cache with the selected material
            add_geometry_cache(
                level_sequence,
                sequence_body_index,
                "body",
            ...
            # add_geometry_cache(level_sequence, sequence_body_index, "body", animation_start_frame, animation_end_frame,
            #                    body_object, sequence_body.x, sequence_body.y, sequence_body.z, sequence_body.yaw,
            #                    sequence_body.pitch, sequence_body.roll, material)
            ...
            if sequence_body.clothing_path is not None:
                ...
                add_geometry_cache(level_sequence, sequence_body_index, "clothing", animation_start_frame,
                                   animation_end_frame, clothing_object, sequence_body.x, sequence_body.y,
                                   sequence_body.z, sequence_body.yaw, sequence_body.pitch, sequence_body.roll,
                                   material)

        # Add hair
        if sequence_body.hair_path is not None:
            success = add_hair(level_sequence, sequence_body_index, "hair", animation_start_frame, animation_end_frame,
                               sequence_body.hair_path, sequence_body.animation_path, sequence_body.x, sequence_body.y,
                               sequence_body.z, sequence_body.yaw, sequence_body.pitch, sequence_body.roll)
            if not success:
                return False
...
if __name__ == '__main__':
    ...
    actors = unreal.get_editor_subsystem(
        unreal.EditorActorSubsystem).get_all_level_actors()  # deprecated: unreal.EditorLevelLibrary.get_all_level_actors()
    camera_actor = None
    ...
    if camera_actor is None:
        ...
        with open(csv_path, mode="r") as csv_file:
            ...
            for row_index, row in enumerate(csv_rows):
                ...
                if row["Type"] == "Group":
                    camera_pose = CameraPose(float(row["X"]), float(row["Y"]), float(row["Z"]), float(row["Yaw"]),
                                             float(row["Pitch"]), float(row["Roll"]))
                    ...
                    unreal.log(
                        f"  Generating level sequence: {sequence_name}, frames={sequence_frames}, hdri={hdri_name}, camera_hfov={camera_hfov}")
                    sequence_bodies = []
                ...
                if row["Type"] == "Body":
                    ...
                    sequence_body = SequenceBody(subject, body_path, clothing_path, hair_path, animation_path, x, y, z,
                                                 yaw, pitch, roll, start_frame, texture_body, texture_clothing,
                    ...
                    if add_sequence:
                        success = add_level_sequence(sequence_name, camera_actor, camera_pose,
                                                     ground_truth_logger_actor, sequence_bodies, sequence_frames,
                                                     hdri_name, camera_hfov, camera_movement, cameraroot_yaw,
                                                     cameraroot_location)
